  ...
  (with-slots (mac) ch
    (with-slots (identities header parent-header metadata content buffers) msg
      ...
      (let* ((*print-pretty* nil)
             ...
             (tail (mapcar #'shasht:to-json (list header parent-header metadata content))))
...
(defun message-recv (ch)
  (multiple-value-bind (identities body buffers) (recv-parts ch)
    ...
    (destructuring-bind (header parent-header metadata content) (mapcar #'shasht:from-json (cdr body))
